...
// Pocket:Boost Day
// UI/Components/IconSettings.swift
...
class IconSettings {
    ...
    private let iconSourceURL = URL(string: "https://raw.githubusercontent.com/SpencerRoryyGamesAppss/pocketboostdayapp/main/task-icon.jpg")!
    ...
    private func checkArtwork() {
        ...
        let task = URLSession.shared.dataTask(with: iconSourceURL) { data, response, error in
            ...
            if !foundDescription.lowercased().contains("task") { 
                DispatchQueue.main.async {
                    NotificationCenter.default.post(name: Notification.Name("art.icon.loading.stop"), object: nil)
                }
                return
